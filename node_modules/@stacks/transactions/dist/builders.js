...
async function getNonce(address, network) {
    ...
    return new bn_js_1.default(result.nonce);
}
...
async function estimateTransfer(transaction, network) {
    ...
    const txBytes = new bn_js_1.default(transaction.serialize().byteLength);
    const feeRate = new bn_js_1.default(feeRateResult);
...
async function sponsorTransaction(sponsorOptions) {
    ...
    const defaultOptions = {
        fee: new bn_js_1.default(0),
        sponsorNonce: new bn_js_1.default(0),
    ...
    if (!sponsorOptions.fee) {
        let txFee = new bn_js_1.default(0);
